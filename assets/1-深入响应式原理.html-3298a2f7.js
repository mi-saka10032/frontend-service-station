...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="数组的响应式处理" tabindex="-1"><a class="header-anchor" href="#数组的响应式处理" aria-hidden="true">#</a> 数组的响应式处理</h3><p>上述的实现只能处理普通对象形式，对于数组仍无法正确处理</p><p>因此，Vue2 底层改写了数组的 7 个方法：push、pop、shift、unshift、splice、sort、reverse</p><p>实现方式：以 <code>Array.prototype</code> 为原型，创建一个 arrayMethods 对象</p><figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/Vue/1672982883181.jpg" alt="响应式原理修改数组方法" tabindex="0" loading="lazy"><figcaption>响应式原理修改数组方法</figcaption></figure><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token comment">// array.js</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> def <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">&quot;./util.js&quot;</span><span class="token punctuation">;</span>
...
代码执行结束
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>调用关系：</p><figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/Vue/1672995344760.jpg" alt="调用关系" tabindex="0" loading="lazy"><figcaption>调用关系</figcaption></figure><p>三个函数相互调用从而形成了递归，与普通的递归有所不同。 有些同学可能会想，只要在 setter 中调用一下渲染函数来重新渲染页面，不就能完成在数据变化时更新页面了吗？确实可以，但是这样做的代价就是：任何一个数据的变化，都会导致这个页面的重新渲染，代价未免太大了吧。我们想做的效果是：数据变化时，只更新与这个数据有关的 DOM 结构，那就涉及到下文的内容了：依赖</p><h2 id="_2-实现订阅-发布模式的依赖收集" tabindex="-1"><a class="header-anchor" href="#_2-实现订阅-发布模式的依赖收集" aria-hidden="true">#</a> 2.实现订阅-发布模式的依赖收集</h2><ul><li>在 Vue2 中，中等粒度依赖，用到数据的组件都是依赖</li><li>在 getter 中收集依赖，在 setter 中触发依赖</li></ul><p><strong>核心：Dep 类和 Watcher 类</strong></p><ul><li>把依赖收集的代码封装成一个 Dep 类，它专门用来管理依赖，每个 Observer 的实例，成员中都有一个 Dep 的实例</li><li>Watcher 是一个中介，数据发生变化时通过 Watcher 中转，通知组件</li><li>下图为官方的订阅-发布模式图例</li></ul><figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/Vue/1672988976577.jpg" alt="发布订阅模式" tabindex="0" loading="lazy"><figcaption>发布订阅模式</figcaption></figure><ul><li>依赖就是 Watcher。只有 Watcher 触发的 getter 才会收集依赖，哪个 Watcher 触发了 getter，就把哪个 Watcher 收集到 Dep 中。</li><li>Dep 使用订阅-发布模式，当数据发生变化时，会循环依赖列表，把所有的 Watcher 都通知一遍。</li><li>代码实现的巧妙之处：Watcher 把自己设置到全局的一个指定位置，然后读取数据，因为读取了数据，所以会触发这个数据的 getter。在 getter 中就能得到当前正在读取数据的 Watcher，并把这个 Watcher 收集到 Dep 中。</li></ul><figure><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/Vue/1672989938316.jpg" alt="更详细的发布订阅模式" tabindex="0" loading="lazy"><figcaption>更详细的发布订阅模式</figcaption></figure><h3 id="初始化订阅-发布系统" tabindex="-1"><a class="header-anchor" href="#初始化订阅-发布系统" aria-hidden="true">#</a> 初始化订阅-发布系统</h3><h3 id="watcher-类初步实现" tabindex="-1"><a class="header-anchor" href="#watcher-类初步实现" aria-hidden="true">#</a> Watcher 类初步实现</h3><p>Watcher 类作为一个独立的订阅系统，当且仅当依赖的数据发生变化时，接收到数据发生变化的消息时，才会触发 update 更新，最后如果回调函数存在再执行回调（对标 watch 配置项中的 handler）</p><p>初步实现如下：</p><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token comment">// watcher.js</span>

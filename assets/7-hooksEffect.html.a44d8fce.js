import{_ as a}from"./_plugin-vue_export-helper.cdc0426e.js";import{o as t,c as p,a as n,b as e,e as o,d as c,r as i}from"./app.2e685bdc.js";const l={},u=c(`<h2 id="副作用的意义" tabindex="-1"><a class="header-anchor" href="#副作用的意义" aria-hidden="true">#</a> 副作用的意义</h2><p><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/React/effectHook.jpg" alt="effectHook" loading="lazy"></p><p>在函数组件主体内（这里指在 React 渲染阶段）改变 DOM、添加订阅、设置定时器、记录日志以及执行其他包含副作用的操作都是不被允许的，因为这可能会产生莫名其妙的 bug 并破坏 UI 的一致性</p><p>使用 useEffect 完成副作用操作。赋值给 useEffect 的函数会在组件渲染到屏幕之后执行。你可以把 effect 看作从 React 的纯函数式世界通往命令式世界的逃生通道</p><p>useEffect 就是一个 Effect Hook，给函数组件增加了操作副作用的能力。它跟 class 组件中的 componentDidMount、componentDidUpdate 和 componentWillUnmount 具有相同的用途，只不过被合并成了一个 API</p><p>该 Hook 接收一个包含命令式、且可能有副作用代码的函数</p><p>注意：</p><ol><li>要避免在 useEffect 的副作用函数中操作 useEffect 绑定的依赖项，这会引起堆栈溢出异常</li><li>effectHook 提供了两种 hook，分为 useEffect 和 useLayoutEffect</li><li>useEffect 在浏览器渲染变更完成后非阻塞执行</li><li>useLayoutEffect 在浏览器渲染变更时阻塞执行</li></ol><h2 id="useeffect" tabindex="-1"><a class="header-anchor" href="#useeffect" aria-hidden="true">#</a> useEffect</h2><p><img src="https://misaka10032.oss-cn-chengdu.aliyuncs.com/React/flushPassiveEffects.jpg" alt="useEffect流程图" loading="lazy"></p><p>useEffect 和其他 hook 函数一样，beginWork 时为 Fiber 挂载 effectHook，二次触发时更新 effectHook，需要注意的是 effect 的 hook 链表和 state、reducer 不是同一个 hook 链表，为的是区分数据流和副作用流</p><h3 id="src-main-jsx" tabindex="-1"><a class="header-anchor" href="#src-main-jsx" aria-hidden="true">#</a> src\\main.jsx</h3><p>src\\main.jsx</p><div class="language-jsx line-numbers-mode" data-ext="jsx"><pre class="language-jsx"><code><span class="token keyword">import</span> <span class="token operator">*</span> <span class="token keyword">as</span> React <span class="token keyword">from</span> <span class="token string">&quot;react/index&quot;</span><span class="token punctuation">;</span>
